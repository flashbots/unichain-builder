...
Here we will start builder playground, configure it to use unichain-builder as an external builder, configure prometheus
and grafana, start unichain-builder and spam the node with transactions using contender. This will give us an end-to-end
manual testing environment.

### Setup

#### 1. Compile unichain-builder in release mode
...
Remember to delete temporary directories created by `builder-playground` and `unichain-builder` between runs, otherwise
...
On other platforms you can figure out the equivalent paths from the logs emitted by `builder-playground` and
`unichain-builder`.
...
#### 2. Run unichain-builder

Run the `release` version of `unichain-builder` binary from this repo with `--builder.playground` flag.
> _`debug` builds will work too but builder's performance will be **severely** degraded._

```shell
RUST_LOG=debug ./target/release/unichain-builder node --builder.playground --metrics=localhost:5559
```
...
```shell
contender spam --tps=150 -l -r http://localhost:2222 --min-balance 1000.5ether
```

Reverting transactions:

```shell
contender spam --tps 30 -l -r http://localhost:2222 --min-balance 1000.5ether revert
```
