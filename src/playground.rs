...
use {
	super::Cli,
	clap::{CommandFactory, parser::ValueSource},
	core::{
...
impl PlaygroundOptions {
	...
	pub fn new(path: &Path) -> Result<Self> {
		...
		let chain =
			OpChainSpecParser::parse(&existing_path(path, "l2-genesis.json")?)?;
	...
	pub fn apply(self, cli: Cli) -> Cli {
		...
		// populate the command line arguments only if they were never set by the
		// user either via the command line or an environment variable. Otherwise,
		// don't override the user provided values.
...
fn read_docker_compose(basepath: &Path) -> Result<serde_yaml::Value> {
	// this happens only once on statup so it's fine to read the file multiple
	// times
	let docker_compose =
		read_to_string(existing_path(basepath, "docker-compose.yaml")?)?;
	serde_yaml::from_str(&docker_compose)
		.map_err(|e| eyre!("Invalid docker-compose file: {e}"))
}

fn extract_service_command_flag(
	basepath: &Path,
	service: &str,
	flag: &str,
) -> Result<String> {
	...
	let args = docker_compose["services"][service]["command"]
		...
		.map(|s| {
			s.as_str().ok_or_else(|| {
				eyre!(
					"docker-compose.yaml service command line argument is not a string"
				)
			})
		})
...
fn extract_authrpc_port(basepath: &Path) -> Result<u16> {
	let builder_url =
		extract_service_command_flag(basepath, "rollup-boost", "--builder-url")?;
	let url =
		Url::parse(&builder_url).map_err(|e| eyre!("Invalid builder-url: {e}"))?;
...
fn extract_trusted_peer_port(basepath: &Path) -> Result<u16> {
	...
	let Some(opgeth_args) =
		docker_compose["services"]["op-geth"]["command"][1].as_str()
	else {
		return Err(eyre!(
	...
	let port_param_position = opgeth_args
		...
		.ok_or_else(|| {
			eyre!("docker_compose: --port param not found on op-geth service")
		})?;
	...
	let Some(opgeth_ports) =
		docker_compose["services"]["op-geth"]["ports"].as_sequence()
	else {
		return Err(eyre!(
	...
	let ports_mapping = opgeth_ports
		...
		.map(|s| {
			s.as_str().ok_or_else(|| {
				eyre!(
					"docker-compose.yaml service ports mapping in op-geth is not a \
					 string"
				)
			})
		})
	...
	let port_mapping = ports_mapping
		...
		.ok_or_else(|| {
			eyre!(
				"docker_compose: external port mapping not found for {port_value} for \
				 op-geth"
			)
		})?;

	// extract the external port from the mapping
	let port_mapping = port_mapping.split(':').nth(1).ok_or_else(|| {
		eyre!("docker_compose: external port mapping for op-geth is not valid")
	})?;
